...
describe("Auth (e2e)", () => {
	...
	it("ignores an invalid credential on an optional-auth route", async () => {
		const response = await request(app.getHttpServer())
			.get("/auth/session")
	...
	it("keeps public metadata available with an invalid bearer token", async () => {
		await request(app.getHttpServer())
			.get("/.well-known/oauth-authorization-server/api/auth")
			.set("authorization", "Bearer invalid")
			.expect(200);
	...
	it("issues and refreshes OAuth tokens through PKCE", async () => {
		...
		try {
			...
			const cookiePrincipal = await request(app.getHttpServer())
				.get("/auth/session")
				.set("cookie", cookie)
				.set("authorization", "Basic ignored")
				.expect(200);
			expect(cookiePrincipal.body.user.id).toBe(userId);
			...
			const encodedPayload = tokenResponse.body.access_token.split(".")[1];
			...
			expect(accessTokenPayload.exp).toBeGreaterThan(
